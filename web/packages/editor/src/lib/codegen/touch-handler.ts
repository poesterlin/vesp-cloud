...
export function generateTouchHandler(project: Project): string {
  ...
  lines.push(`      // Call Home Assistant service`);
  lines.push(`      // This uses the ESPHome native API`);
  lines.push(`      {`);
  lines.push(`        auto call = homeassistant::HomeAssistantServiceCall();`);
  lines.push(`        call.set_service(box.action.serviceCall.service);`);
  lines.push(`        // Parsing dataJson and adding entityId would go here`);
  lines.push(`        // For simplicity in MVP, we might just use a hardcoded helper`);
  lines.push(`      }`);
  lines.push(`      break;`);
  lines.push(`  }`);
  lines.push(`}`);
